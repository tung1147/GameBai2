...
var InviteDialog = Dialog.extend({
    ctor: function () {
        ...
        this.initWithSize(cc.size(720, 480));

        var listItem = new newui.TableView(cc.size(625, 335), 2);
        listItem.setScrollBarEnabled(false);
        listItem.setMargin(20, 20, 0, 0);
        listItem.setPadding(20.0);
        listItem.setPosition(50, 50);
        this.addChild(listItem);
        this.listItem = listItem;

        var noPlayerLabel = new cc.LabelBMFont("Hiện tại không có người chơi trong sảnh, vui lòng chờ", cc.res.font.Roboto_Condensed_30, this.getContentSize().width - 200, cc.TEXT_ALIGNMENT_CENTER);
    ...
    addItem: function (avt, username, gold) {
        ...
        if (displayUserName.length > 3 && (displayUserName != PlayerMe.username)) {
            displayUserName = displayUserName.substring(0, displayUserName.length <= 10?displayUserName.length - 3:10) + "***";
        }
